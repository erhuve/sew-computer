...
test('diagnostic torso equilibrium closes source seams without resizing fabric', async () => {
  const python = process.env.SEW_ENGINE_PYTHON || join(import.meta.dir, '.venv/bin/python');
  const child = Bun.spawn([python, join(import.meta.dir, '../../scripts/test_solver_torso_control.py')], { stdout: 'pipe', stderr: 'pipe', env: { PATH: '/usr/bin:/bin', OPENBLAS_NUM_THREADS: '1' } });
