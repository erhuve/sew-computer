...
class IpcSurfaceContact:
    ...
    def hessian(self, positions):
        ...
        positions, collisions = self._collisions(positions)
        hessian = self.potential.hessian(collisions, self.mesh, positions, ipctk.PSDProjectionMethod.CLAMP)
    ...
    def profile(self):
        return {"adapter": "experimental-ipc-surface-contact-v2", "ipctk": "1.6.0",
                "activationDistanceM": self.activation_distance_m,
                "minimumDistanceM": self.minimum_distance_m, "stiffness": self.stiffness,
                "physicalBarrier": False, "seamExclusions": False, "accepted": False,
                "ccd": {"method": "TightInclusionCCD", "tolerance": 1e-6,
