...
capture_verifier.verify_capture(run, report)
...
from solver_attempt_journal import recover_attempt_journal, strict_loads
...
source = strict_loads((run / "canonical.json").read_text())
placement = strict_loads((run / "placement.json").read_text())
...
if sewing_enabled:
    assert arguments["target_fraction"] == 1
    assert arguments["subdivisions"].bit_length() - 1 + arguments["max_depth"] <= 40
    from solver_sewing_input import bind_sewing_activation
    sewing_controls, binding = bind_sewing_activation(source, arguments["subdivisions"],
                                                       sewing_mode=arguments["sewing_mode"])
    assert json.dumps(report["sewingBinding"], sort_keys=True, separators=(",", ":"), allow_nan=False) == json.dumps(
        binding, sort_keys=True, separators=(",", ":"), allow_nan=False)
    sewing_record = sewing_verifier.derive_sewing(source, arguments["subdivisions"],
                                                 sewing_mode=arguments["sewing_mode"])
    initial_sewing_verification = sewing_verifier.verify_initial(sewing_record, initial, report)
schedule_knots = None
...
def operation_progress(fraction, key):
    ...
    if sewing_enabled:
        amount = (fraction - lower["fraction"]) / (upper["fraction"] - lower["fraction"])
        return lower[key] + amount * (upper[key] - lower[key])
...
solver = GlobalSewingSolver(model, rows, sewing_controls.compliance if sewing_enabled else 1e-8, contact=contact, fold_barrier_joules=1e-5,
...
if sewing_enabled:
    assert rows == sewing_controls.rows
...
for artifact in report["acceptedStateArtifacts"]:
    ...
    if sewing_enabled:
        ...
        old_targets = sewing_verifier.targets(sewing_record, old_sewing_progress)
        np.testing.assert_array_equal(targets, sewing_verifier.targets(sewing_record, sewing_progress))
        sewing_gradient, sewing_evidence = sewing_verifier.verify_sewing_step(
            sewing_record, previous, positions, record["startFraction"], record["endFraction"],
    ...
    if gripper_enabled or sewing_enabled:
        old_sewing_progress = operation_progress(previous_fraction, "sewingProgress")
        ...
        if gripper_enabled:
            ...
            energy_options.update(previous_gripper_targets=old_grip_targets, gripper_targets=grip_targets,
                                  previous_gripper_activation=old_grip_activation, gripper_activation=grip_activation)
        if sewing_enabled:
            energy_options.update(previous_sewing_activation=sewing_verifier.parameters(sewing_record, previous_fraction),
                                  sewing_activation=sewing_verifier.parameters(sewing_record, record["endFraction"]))
        if fold_recipe:
        ...
        gripper_diagnostics["energyBalanceVerification"] = (
            "Captured helper reconstructs other energy terms from adjacent states; enabled sewing and gripper forces/work are independently checked by current verifiers")
    ...
    results.append({"endFraction": record["endFraction"], "recomputedResidualN": residual,
        ...
        **fold_diagnostics, **gripper_diagnostics, **sewing_diagnostics})
...
verified_sewing_summary = None
if sewing_enabled:
    sewing_work_fields = ("sewingParameterWorkJoules", "sewingTargetParameterWorkJoules",
    ...
    other_work_fields = ("externalParameterWorkJoules", "mechanicalChangeJoules", "mechanicalChangeMinusParameterWorkJoules")
    verified_sewing_summary = {"acceptedSteps": len(results),
        **{key: math.fsum(item["sewingVerification"][key] for item in results) for key in sewing_work_fields},
        **{key: math.fsum(energy[key] for energy in verified_energy_balances) for key in other_work_fields},
        "scope": "Recovered accepted prefix; seam control work independently reconstructed under sampled binary64 geometry. Other terms use captured helpers; no construction phase acceptance."}
    if report.get("sewingWorkSummary") is not None:
        work_summary = report["sewingWorkSummary"]
        assert type(work_summary["acceptedSteps"]) is int and work_summary["acceptedSteps"] == len(results)
        for key in (*sewing_work_fields, *other_work_fields):
            assert type(work_summary[key]) in (int, float) and math.isfinite(work_summary[key])
            if key in sewing_work_fields:
                assert work_summary[key] == verified_sewing_summary[key]
            else:
                np.testing.assert_allclose(work_summary[key], verified_sewing_summary[key], rtol=1e-12, atol=1e-14)
    elif report["completed"]:
        raise ValueError("Completed sewing activation run requires accepted-only work summary")
