...
Both connections make one provider request per attempt, with a 120-second deadline, a 48 KB design-text limit, at most three sanitized 768-pixel references and a persistent 12-request/hour submission limit. Only one interpretation attempt runs across API instances sharing the database. The HTTP request acknowledges a durable job promptly; the browser polls instead of waiting past the proxy deadline. An interrupted attempt can replay once after lease expiry or shutdown, potentially charging twice; validation/provider failures do not automatically replay. Body fields, size label and private callouts are excluded; brief/BOM text and selected photos can still contain personal information. The dedicated API caps completion tokens at 6,000. Codex does not accept a provider-side output-token cap: its transport instead bounds the answer at 32 KB and the SSE stream at 4 MB, and consumes the owner's Codex allowance. There is no provider-enforced dollar ceiling on this path; the UI states that limitation. Proposals persist for review and become stale after any draft change. Jobs and completed proposals can be recovered by reopening the project.

The pending implementation upgrades older databases to SQLite schema 5. Schema 2 adds proposal and request-budget tables; schema 3 adds journaled reusable-measurement deletion; schema 4 adds durable interpretation jobs; schema 5 adds independent private 3D jobs and artifacts. Existing projects, sessions, credentials and pattern bytes are preserved. Older application binaries refuse schema 5: rollback requires a schema-5-capable build, not a pragma downgrade or stale-data restore. Disable model configuration to return to manual operation while retaining current storage and deletion handling. These pending lifecycle changes are not a claim of production deployment.

The pending 3D viewer exposes flat pattern-derived placement inspection only. Its fixed trusted Python worker drops root privileges and enforces measured resource limits, strips credentials and records seccomp network-denial availability. It does not provide filesystem isolation; arbitrary code remains disabled. All mesh/report requests require the studio session, including loopback access. Both files reveal dimensions and are excluded from all existing exports. No assembled garment, body contact, material-aware drape or physical-fit validation is implied by a successful inspection job. See [API lifecycle and controls](../apps/api/README.md#private-3d-placement-inspection).
...
New generations also include `pattern-a4-tiled.pdf` and `pattern-letter-tiled.pdf`. Exported filenames preserve the paper format and add the immutable artifact ID. Select the matching paper, print the guide at Actual size / 100% with scaling disabled, and measure both sides of its 100 mm square before printing remaining sheets. Each piece has its own row/column grid and 10 mm overlap. Old revisions retain their original file inventory; exporting never regenerates them. Both tiled PDFs are excluded when pattern disclosure is off. Tiling is digitally checked but physical printer accuracy and garment fit remain unvalidated.

The component shirt pipeline was privately released on 2026-09-16 at application commit `284a261`. Existing designs and saved patterns are preserved. For a previously interpreted base shirt, open Design and interpret the saved brief again, review/accept the new construction choices, then Save & generate. Interpretation adds requirements without deleting earlier unsupported rows; review those rows explicitly rather than assuming the new proposal resolved them. New designs can use the component flow immediately.

Component pipeline deployments also require rerunning `python3 scripts/setup-engine.py --install` in the target checkout: the locked geometry runtime now includes Shapely 2.1.2 for cut-line offsets and polygon checks. Existing revisions omit the new optional construction fields and remain readable without rewriting their digests. New component artifacts require the matching application code; do not roll back to a build that rejects their structured annotations. See [scope and verification](reviews/garment-pipeline.md).
