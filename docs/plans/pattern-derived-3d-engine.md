...
The final numerical/supervision regression run passes **842 tests in 379.840 seconds** on pinned Linux ARM, including all twenty-two new fold-control/audit cases. This verifies the offline checkpoint with the proposed refined runner/main-replay integration still unapplied, not garment construction or an application release.
