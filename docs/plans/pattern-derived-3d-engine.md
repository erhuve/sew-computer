...
Allowance-crease continuation, 2026-09-22: the offline cuff extractor can subdivide the cut mesh along its recorded straight outer stitch line, explicitly extending through both side allowances to the cut boundary. Original vertices, parent triangles and interpolation weights are retained; the source metric and outline are preserved. This supplies actual allowance-aligned hinges to the existing supervised fold actuator. It does not execute sewn shell/facing turning, corner treatment, binding or full-shirt assembly. See the [allowance control](../research/3d-solver-feasibility.md#source-cuff-allowance-crease--2026-09-22). No deployment is enabled.
