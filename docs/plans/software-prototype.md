...
Current owner direction, 2026-09-26: expand the connected MVP beyond shirts to relaxed dresses and elastic-waist skirts, and simplify the overall interface from creation through export. Optional starting shapes must remain editable; descriptions and unsupported original intent remain first-class. The [current multi-family checkpoint](../reviews/multiple-garments.md) supersedes the static-demo next actions below. The three-family implementation is verified locally across creation, edits, sizing, previews, persistence and matching downloads. Next collect participant feedback on these connected flows; no arbitrary-garment or physical-readiness claim follows from this bounded scope.

The owner's first visual feedback rejected the rigid-panel arrangement: the MVP must still show a realistic-looking garment even when physics is approximate. The default demo now uses a posed cloth preview derived from source patterns, with explicit approximation limits. This is the first recorded owner reaction, not evidence of external demand. Visual quality must be assessed from the rendered result, not inferred from passing tests.
