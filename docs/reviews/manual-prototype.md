...
- The private Zo service is published at https://sew-computer-hatsunemiku.zo.computer. A fresh release pass ran all 90 tests, typecheck and build. Unauthenticated tunnel requests redirect to Zo sign-in; direct unauthenticated project requests return 401. A browser smoke check against the actual production process verified owner login, real CPU panels and a valid PDF export, then deleted its synthetic project. That browser check routed the production hostname to the local process; an owner-authenticated journey through the external tunnel was not available to the test browser. See [deployment instructions](../deployment.md).
