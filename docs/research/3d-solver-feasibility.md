...
Local inspection of the pinned Newton bending kernel identifies a bounded next implementation: elastic four-vertex dihedral hinges with energy `0.5 * edge_ke * edge_rest_length * (theta - theta_rest)^2`. This is not implemented in the global reference yet. Its sparse search metric must include cross-vertex hinge coupling; Newton's Gauss–Newton outer product is not an exact hinge Hessian. Principal-angle branch crossings, degenerate hinges, mirror orientation, force/torque invariance and independent derivatives need explicit tests before integration. Conservative bending alone cannot establish physical damping or settled drape.
