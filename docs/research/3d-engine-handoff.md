...
[Compliant material controls](3d-solver-feasibility.md#compliant-source-material-controls--2026-09-23) now supply source-triangle pulling, explicit engagement/release, force and discrete-work accounting, rollback, capture and independent gripper replay. The [retained two-triangle contact diagnostic](3d-material-gripper-contact-results.json) completes and replays eight states; it remains synthetic and unaccepted. Independent precision review repaired force, directional-energy and torque cancellation. The direct sewing primitives described below address pending forces and engagement energy; captured per-member execution remains unfinished. Construction-specific binding/turning paths must also preserve material-side, interfacing, continuous-seam and opening-passage gates. Generic pulling controls do not execute those phases.

The direct [seam activation primitives](3d-solver-feasibility.md#pending-seam-mechanics--2026-09-23) now retain all canonical rows, suppress pending forces and stiffness, and report unweighted active errors. A standalone monotone schedule and endpoint work helper are implemented. Next bind the complete source rows and explicit controls to captured input, then integrate adaptive sampling/rollback, CLI capture and independent replay. Raw activation passed to the existing adaptive runner fails closed. No construction phase has executed.
