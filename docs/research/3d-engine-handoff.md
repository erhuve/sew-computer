...
The complete pinned Linux numerical/supervision suite now passes **820 tests in 361.034 seconds**, including all twenty-four new gripper migration/audit cases and the preceding forty seam-remap/source-builder/integration cases. Log: `.planning/solver/binding-gripper-full-numerical-sep23-v1.log`. Independent review verifies the retained migration, both counterexamples and every artifact/helper identity after adding the initially omitted seam-verifier digest. This verifies the implemented helpers and existing paths while the two proposed entry-point edits remain unapplied. Earlier counts below are historical.
...
The [binding refinement descriptor](3d-solver-feasibility.md#binding-crease-preprocessing--2026-09-23) adds attachment/finish crease chains through the complete original cut domain, yielding 29 strip vertices and 44 triangles with rationally composed original-parent lineage. It is deliberately non-executable. Seam and gripper correspondence now have separate witnesses; explicit crease-side frames and source capture/replay remain pending. Preserve the immutable original five-fabric unit and all forty source rows. Flat point containment alone cannot establish coefficient preservation under deformation; do not normalize, clamp or change the source profile to bypass validation.
