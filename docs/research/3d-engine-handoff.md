...
Current continuation: source-bound seam/gripper remapping, native fold controls and refined rigid placement now have separate independent audits. The latest complete pinned Linux suite passes **866 tests**. Static placement additionally passes fixed-profile contact admission, complete candidate coverage and exact stationary primitive/triangle checks. Captured refined motion remains unexecuted while the proposed runner/main-replay edits await specific authorization after automatic approval rejection. Construction phases, settling, material/refinement validation and full-shirt integration remain open. The [numerical adversarial review](../reviews/3d-cloth-contact.md#numerical-continuation-review--2026-09-23) and [current solver evidence](3d-solver-feasibility.md#source-bound-refined-placement--2026-09-23) supersede historical checkpoint counts and model-review status below; no garment is accepted.
...
The latest complete pinned Linux numerical/supervision suite passes **866 tests in 407.172 seconds**, including all twenty-four new placement producer/audit cases; those twenty-four also pass on macOS. Log: `.planning/solver/binding-placement-full-numerical-sep23-v1.log`. Earlier counts below are historical checkpoints. Both proposed capture/main-replay entrypoints remain unchanged and their prepared patch still passes `git apply --check`; no application release check or deployment is included.
