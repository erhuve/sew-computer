...
Current continuation: [per-hinge fold activation and release](3d-solver-feasibility.md#guarded-fold-activation-and-release--2026-09-23) now enter the generic guarded direct solver and adaptive retries. A synthetic two-layer control completes all 32 steps, with unchanged rest-array checks, sequential release and nonzero contact response; its final 77 mm/s speed is not settled. Source-bound seam/gripper remapping, native fold controls and refined placement retain their separate audits, but the refined construction schedule and captured motion are still unexecuted. The proposed runner/main-replay edits await specific authorization after automatic approval rejection. Construction phases, material/refinement/settling validation and full-shirt application integration remain open. No garment is accepted.
...
1. **Bind and verify source construction controls.** Author the temporal fold/gripper recipe against audited source, native-hinge and placement identities. Complete captured integration and independent replay after the pending entry-point authorization. Then resolve facing hinge tracking, seam/frame reactions and contact interactions on the source cuff. Require measured target error, seam error, layer-side behavior and deformation, not just completed time or low residual; the generic two-layer control does not establish construction.
