...
The complete pinned Linux numerical/supervision suite now passes **796 tests in 422.720 seconds**, including the forty new remap, independent audit, source-builder and binder integration cases. Log: `.planning/solver/binding-remap-full-numerical-sep23-v1.log`. This verifies the implemented helpers and existing paths while the two proposed entry-point edits remain unapplied. Earlier counts below are historical.
...
The [binding refinement descriptor](3d-solver-feasibility.md#binding-crease-preprocessing--2026-09-23) adds attachment/finish crease chains through the complete original cut domain, yielding 29 strip vertices and 44 triangles with rationally composed original-parent lineage. It is deliberately non-executable. Next implement separately derived numerical anchors with coefficient-pullback witnesses, explicit crease-side frames and gripper rebinding, then connect source capture/replay. Preserve the immutable original five-fabric unit and all forty source rows. Flat point containment alone cannot establish coefficient preservation under deformation; do not normalize, clamp or change the source profile to bypass validation.
