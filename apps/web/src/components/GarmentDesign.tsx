...
export default function GarmentDesign({doc,geometry=null,onChange,compact=false}:{doc:GarmentDocument;geometry?:PatternGeometry|null;onChange?:(doc:GarmentDocument)=>void;compact?:boolean}) {
  ...
  const change=(key:keyof ShirtDesign,value:unknown)=>{
    const next=ShirtDesignSchema.safeParse({...design,[key]:value,...(key==='opening'&&value==='none'?{collar:'none',frill:'none'}:{}),...(key==='sleeves'&&value==='short'?{sleeveLengthMm:220,cuff:'none'}:key==='sleeves'&&value==='long'?{sleeveLengthMm:550}:key==='sleeves'&&value==='none'?{cuff:'none'}:{})});
  ...
  return <section className="garment-design" aria-label="Garment construction">
    {!compact&&<div className="garment-flats">{garmentFlats(doc,geometry).map(flat=>{const extent=Math.max(...flat.lines.flatMap(line=>line.points.map(point=>Math.abs(point[0]))))+20;return <figure key={flat.view}>
    ...
    {!compact&&<p className="fineprint">Construction drawing · proportions illustrative</p>}
    ...
    {onChange&&<div><h3>Make it yours</h3><div className="construction-controls">
      {choices.map(([key,label,options])=><label key={key}>{label}<select aria-label={label} value={String(design[key])} onChange={event=>change(key,event.target.value)}>{options.map(value=><option key={value} value={value}>{choiceNames[value]??value}</option>)}</select></label>)}
      </div><details><summary>Fine-tune dimensions</summary><div className="construction-controls">{numbers.map(([key,label,min,max,step])=><label key={key}>{label}<input aria-label={label} type="range" min={min} max={max} step={step} value={Number(design[key])} onChange={event=>change(key,Number(event.target.value))}/><output>{Number(design[key])}</output></label>)}
    </div></details></div>}
